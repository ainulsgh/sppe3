// resources/js/Pages/dpmptsp/Data.jsx
...
export default function Data({ records = [], filters = {} }) {
  ...
  const indikator = [
    { label: 'Persetujuan Bangunan Gedung',                 key: 'pbg',           unit: 'Unit' },
  ];

  return (
    <AuthenticatedLayout header={<span>Dinas Penanaman Modal dan Pelayanan Terpadu Satu Pintu</span>}>
      <Head title="DPMPTSP" />
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-3" ref={exportRef}>
            <Link
              href={`/dpmptsp/edit?tahun=${selectedYear ?? ''}&bulan=${selectedMonth ?? ''}`}
            ...
            <div className="relative">
              ...
              {showExport && canAct && (
                <div className="absolute right-0 mt-2 w-44 bg-white rounded-md shadow-lg border z-10">
                  <a
                    href={`/dpmptsp/export?format=csv&tahun=${selectedYear}&bulan=${selectedMonth}`}
                    className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                  >
                    Export CSV
                  </a>
                  <a
                    href={`/dpmptsp/export?format=xls&tahun=${selectedYear}&bulan=${selectedMonth}`}
                    className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                  >
                    Export Excel
                  </a>
