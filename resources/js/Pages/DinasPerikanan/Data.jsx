// resources/js/Pages/DinasPerikanan/Data.jsx
...
export default function Data({ records = [], filters = {} }) {
  ...
  const indikator = [
    ...
    { label: 'Budidaya Laut (Rumput Laut)',      key: 'budidaya_laut_rumput_laut',      unit: 'Ton' },
    { label: 'Budidaya Tambak (Rumput Laut)',    key: 'budidaya_tambak_rumput_laut',    unit: 'Ton' },
    { label: 'Budidaya Tambak (Udang)',          key: 'budidaya_tambak_udang',          unit: 'Ton' },
    { label: 'Budidaya Tambak (Bandeng)',        key: 'budidaya_tambak_bandeng',        unit: 'Ton' },
    { label: 'Budidaya Tambak (Ikan Lainnya)',   key: 'budidaya_tambak_lainnya',        unit: 'Ton' },
    { label: 'Budidaya Kolam',                   key: 'budidaya_kolam',                 unit: 'Ton' },
    { label: 'Budidaya Sawah',                   key: 'budidaya_sawah',                 unit: 'Ton' },
  ];
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-3" ref={exportRef}>
            <Link
              href={`/perikanan/edit?tahun=${selectedYear ?? ''}&bulan=${selectedMonth ?? ''}`}
            ...
            <div className="relative">
              ...
              {showExport && canAct && (
                <div className="absolute right-0 mt-2 w-44 bg-white rounded-md shadow-lg border z-10">
                  <a
                    href={`/perikanan/export?format=csv&tahun=${selectedYear}&bulan=${selectedMonth}`}
                    className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                  >
                    Export CSV
                  </a>
                  <a
                    href={`/perikanan/export?format=xls&tahun=${selectedYear}&bulan=${selectedMonth}`}
                    className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                  >
                    Export Excel
                  </a>
        ...
        <div className="overflow-x-auto shadow border">
          <table className="w-full text-xs md:text-sm leading-tight">
            <thead className="bg-slate-900 text-white">
              <tr>
                <th className="px-2 py-3 text-center font-semibold border w-1/2">Indikator</th>
                <th className="px-2 py-3 text-center font-semibold border">Satuan</th>
                <th className="px-2 py-3 text-center font-semibold border">Jumlah</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {indikator.map((row) => (
                <tr key={row.key} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100 text-sm">
                  <td className="px-2 py-3 text-slate-700">{row.label}</td>
                  <td className="px-2 py-3 text-center text-slate-900 border">{row.unit}</td>
                  <td className="px-2 py-3 text-center font-medium font-mono border">
                    {record
